  ...
  return (
    <div ref={ref} className="w-[80mm] p-4 text-black bg-white font-mono text-sm leading-tight mx-auto border" style={{ printColorAdjust: 'exact' }}>
      ...
      <div className="text-center mb-4">
        ...
        {companyPhone && <p className="text-sm">Tel: {companyPhone}</p>}
      </div>

      {/* Meta Info */}
      <div className="mb-4 text-sm border-b border-black pb-2 border-dashed">
        ...
        {customerName && <div className="flex justify-between"><span>Customer:</span> <span className="font-semibold text-right max-w-[60%]">{customerName}</span></div>}
      </div>
      ...
      {/* Totals */}
      <div className="border-t border-black pt-2 border-dashed text-sm space-y-1">
      ...
      <div className="text-center mt-6 text-sm font-bold">
        <p>{footerMessage}</p>
        <p className="mt-2 text-xs font-normal">Powered by EPOS</p>
      </div>
    </div>
  );
});
